...
import type { MarketSignalItem, MarketSignalSeverity, MarketSignalSourceType, MarketSignalStatus, MarketSignalType } from "@/lib/types"
    ...
    async function loadSignals() {
      ...
      try {
        ...
        if (res.ok) {
          ...
          console.warn("Failed to load signals from API, using demo data")
          setRows([])
          setDataState("ready")
        } else {
          console.error("Failed to load signals from API:", res.status)
          setRows([])
          setDataState("error")
        }
      } catch (err) {
          ...
          {photos.length > 1 && (
            <div className="grid grid-cols-4 gap-2">
              {photos.slice(1, 5).map((photo, idx) => (
                <div key={idx} className="relative h-16 overflow-hidden rounded-lg">
                  <Image
                    src={photo}
                    alt={`${propertyTitle} photo ${idx + 2}`}
                    fill
                    className="object-cover"
                    sizes="150px"
                    onError={(e) => { e.currentTarget.style.display = "none" }}
