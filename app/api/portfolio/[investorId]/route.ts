...
import { mockHoldings, getHoldingProperty } from "@/lib/real-estate"
...
export async function GET(
  ...
  try {
    // Try to fetch from DB first, fall back to mock
    let holdings = await getHoldingsByInvestor(investorId)
    let useMock = false

    if (holdings.length === 0) {
      useMock = true
      // Use mock holdings
      const mockForInvestor = mockHoldings.filter((h) => h.investorId === investorId)
      holdings = mockForInvestor.map((h) => ({
        id: h.id,
        tenantId: "tenant-1",
        investorId: h.investorId,
        listingId: h.propertyId,
    ...
    const enrichedHoldings = await Promise.all(
      holdings.map(async (holding) => {
        const property = useMock ? getHoldingProperty(holding as unknown as import("@/lib/real-estate").PropertyHolding) : null
    ...
    const summary = {
      ...
      dataSource: useMock ? "mock" : "database",
    }
