...
export const propertyEvaluationSchema = z.object({
  property: z.object({
    ...
    completionStatus: z.enum(["ready", "off_plan", "under_construction", "unknown"]).optional(),
    ...
    purpose: z.enum(["for-sale", "for-rent"]).nullable().optional(),
    buildingName: z.string().nullable().optional(),
    buildingFloors: z.number().int().min(0).nullable().optional(),
    totalParkingSpaces: z.number().int().min(0).nullable().optional(),
    elevators: z.number().int().min(0).nullable().optional(),
    floorPlanImages: z.array(z.string()).optional(),
    paymentPlan: z.object({
      downPaymentPercent: z.number().nullable().optional(),
      preHandoverPercent: z.number().nullable().optional(),
      handoverPercent: z.number().nullable().optional(),
      postHandoverPercent: z.number().nullable().optional(),
    }).nullable().optional(),
