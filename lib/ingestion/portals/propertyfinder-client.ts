...
/**
 * PropertyFinder API Client
 * -------------------------
 * Client for fetching listing data from PropertyFinder portal.
 * 
 ...
 * 1. Official PropertyFinder API partnership
 * 2. Web scraping solution
 * 3. Third-party data provider
 */

const PF_API_BASE_URL = process.env.PROPERTYFINDER_API_BASE_URL || 'https://api.propertyfinder.ae/v1'
const PF_API_KEY = process.env.PROPERTYFINDER_API_KEY || ''
...
/**
 * Fetch listings from PropertyFinder API
 */
export async function fetchPropertyFinderListings(params: PortalQueryParams): Promise<{
  ...
  if (params.area) queryParams.set('location_slug', params.area.toLowerCase().replace(/\s+/g, '-'))
  if (params.propertyType) queryParams.set('property_type', params.propertyType.toLowerCase())
  if (params.listingType) queryParams.set('listing_type', params.listingType)
  ...
  if (params.bedrooms) queryParams.set('bedrooms', String(params.bedrooms))
  queryParams.set('limit', String(params.limit ?? 100))
  queryParams.set('offset', String(params.offset ?? 0))
  
  const url = `${PF_API_BASE_URL}/listings?${queryParams.toString()}`
  ...
  try {
    const response = await withRetry(
      async () => {
        const res = await fetch(url, {
          method: 'GET',
          headers: {
            'Authorization': PF_API_KEY ? `Bearer ${PF_API_KEY}` : '',
        ...
        if (!res.ok) {
          ...
          throw new Error(`PropertyFinder API error: ${res.status} - ${errorText}`)
        }
    ...
    console.error('[PropertyFinder Client] Fetch error:', error)
    return {
      listings: [],
      total: 0,
      hasMore: false,
 ...
 */
export async function fetchAllPropertyFinderListings(params: {
  areas?: string[]
  ...
  for (const area of areas) {
    ...
    while (hasMore) {
      const result = await fetchPropertyFinderListings({
        area,
      ...
      if (result.error) {
        console.warn(`[PropertyFinder] Error fetching ${area}: ${result.error}`)
        break
      }
 ...
 * Test API connection
 */
export async function testPropertyFinderConnection(): Promise<{
  success: boolean
  message: string
}> {
  try {
...
/**
 * Generate mock PropertyFinder listings for development/testing
 */
export function generateMockPropertyFinderListings(count: number = 100): PortalApiListing[] {
  const areas = [
  ...
  for (let i = 0; i < count; i++) {
    ...
    // Generate realistic price (slightly different from Bayut for variety)
    const basePrice = isVilla ? 3200000 : 1050000 + bedrooms * 520000
    const areaMultiplier = area === 'Palm Jumeirah' ? 1.95 : area === 'Downtown Dubai' ? 1.65 : 1
    const price = Math.round(basePrice * areaMultiplier * (0.8 + Math.random() * 0.4))
    
    // Generate size
    const baseSqft = isVilla ? 2600 : 520 + bedrooms * 410
    ...
    const hasPriceCut = Math.random() > 0.7
    const originalPrice = hasPriceCut ? Math.round(price * (1.05 + Math.random() * 0.15)) : undefined
    
    listings.push({
      listing_id: `PF-MOCK-${i + 1}`,
      portal: 'PropertyFinder',
      title: `${bedrooms} Bedroom ${propType} for Sale in ${area}`,
      location: {
        area,
        community: area,
        city: 'Dubai',
      ...
      is_active: true,
      is_verified: Math.random() > 0.4, // TruCheck
      verification_status: Math.random() > 0.4 ? 'trucheck_verified' : 'pending',
      furnishing: Math.random() > 0.6 ? 'Unfurnished' : 'Furnished',
      completion_status: Math.random() > 0.85 ? 'Off-Plan' : 'Ready',
    })
  }
