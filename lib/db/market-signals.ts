...
export async function listMarketSignalsFeed(args: { tenantId: string; limit?: number }): Promise<MarketSignalItem[]> {
  ...
  const { data, error } = await supabase
    .from("market_signal")
    .select(
      "id, org_id, created_at, source_type, source, timeframe, type, severity, status, geo_type, geo_id, geo_name, segment, metric, current_value, prev_value, delta_value, delta_pct, confidence_score"
    )
